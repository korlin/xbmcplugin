...
class Xbmcearth_communication:
	...
	def get_GoogleStreetView(self, url, param):
		header = {
			"Host":"maps.google.com",
			"User-Agent":"Mozilla/5.0 (Windows; U; Windows NT 5.1; de; rv:1.8.1.11) Gecko/20071127 Firefox/2.0.0.11",
			"Accept":"*/*",
		...
		print params
		body = ''
		self.sTargetUrl = "http://maps.google.com/cbk"
		if self._request("GET",params,header, body)!=True:
			return False
		data = self.sData
		fName = file
		f = open(TEMPFOLDER + "streetview.xml", 'wb')
		f.write(data)
		f.close()
		return self.sData
...
class get_file(Thread):
	...
	def run(self):	
		...
		if not os.path.exists(TEMPFOLDER + self.filename) or self.cache == 0:
			...
			params = self.parms
			body = ''
			if self._request("GET",params, body)!=True:
				return
			data = self.sData
			fName = file
	...
	def _request(self, action, params, body ):
		host = self.sTargetUrl[self.sTargetUrl.find('//')+2:self.sTargetUrl.find('/',self.sTargetUrl.find('//')+2)]
		header = {
			"Host": host,
			"User-Agent":"Mozilla/5.0 (Windows; U; Windows NT 5.1; de; rv:1.8.1.11) Gecko/20071127 Firefox/2.0.0.11",
			"Accept":"*/*",
		...
		if response.status != 200:
			if response.status == 307:
				...
				nodelist = xmldoc.getElementsByTagName("Endpoint")
				for node in nodelist:
					temp_url = self.sTargetUrl[self.sTargetUrl.find('://')+3:len(self.sTargetUrl)]
					temp_url = temp_url[temp_url.find('/'):len(temp_url)]
					self.sTargetUrl = "http://" + node.firstChild.data + temp_url
					print "redirect - " + self.sTargetUrl + params
					self._request("GET",params, body)
				return True
			else:
				print response.status, response.reason
				print response.read()
