...
class MainClass(xbmcgui.WindowXML):
	...
	def __init__(self, *args, **kwargs):
		...
		self.xbmcearth_communication.get_Google_Analytics( referer_url,  urllib.quote(("Start Version:" + str(__version__)).encode('utf-8')), "/xbmc_earth/start.html", self)
	...
	def onInit(self):
		#setup all the sat Pictures
		self.map_pos_x_windowed = self.getControl(511).getPosition()[0] + self.getControl(511).getWidth()
		self.map_pos_y_windowed = self.getControl(511).getPosition()[1] + self.getControl(511).getWidth()
		self.pic_size_windowed = self.getControl(511).getWidth()
		...
		for x in range(self.map_size_x):
			...
			for y in range(self.map_size_y):
				...
				self.satBlocks[x].append(self.getControl(510+i))
		...
		for x in range(self.map_size_x):
			...
			for y in range(self.map_size_y):
				...
				Map_pos_x = (self.map_pos_x - Pic_size) + (Pic_size*x)
				Map_pos_y = (self.map_pos_y - Pic_size) + (Pic_size*y)
				self.mapBlocks[x].append(self.getControl(520+i))
		...
		#Position of the Route Object
		self.route_pic = self.getControl(530)
		self.route_pic.setPosition(self.map_pos_x-self.pic_size,self.map_pos_y-self.pic_size)
		self.route_pic.setHeight(Pic_size*3)
		self.route_pic.setWidth(Pic_size*3)
		self.route_pic.setVisible(False)
		#Position of the Nasas OnEarth Object
		self.nasa_view = self.getControl(540)
		self.nasa_view.setPosition(self.map_pos_x-self.pic_size,self.map_pos_y-self.pic_size)
		self.nasa_view.setHeight(Pic_size*3)
		self.nasa_view.setWidth(Pic_size*3)
		self.nasa_view.setVisible(False)
		...
		for button_id in range( 100, 108 ):
			try:
				...
				pass
		for button_id in range( 109, 123 ):
			try:
				self.getControl( button_id ).setLabel( __language__( button_id ) )
			except:
				pass
	...
	def onClick(self, controlID):
		...
		if controlID == 100:
			...
			con = google_streetview.GUI( "script-%s-google_streetview.xml" % ( __scriptname__.replace( " ", "_" ), ), os.getcwd(),  "Default", 0,mainWindow = self)
			del con		
	...
	def search_Route(self):
		...
		if keyboard.isConfirmed():
			...
			import chooser
			ch = chooser.GUI( "script-%s-chooser.xml" % ( __scriptname__.replace( " ", "_" ), ), os.getcwd(),  "Default", 0, choices=result["Placemarks"], descriptions=result["Placemarks"], original=-1, selection=0, list_control=1, title=urllib.quote(__language__( 10004 ).encode('utf-8')) )
		...
		if keyboard.isConfirmed():
			...
			result = self.parse_kml(search_kml)
			import chooser
			ch = chooser.GUI( "script-%s-chooser.xml" % ( __scriptname__.replace( " ", "_" ), ), os.getcwd(),  "Default", 0, choices=result["Placemarks"], descriptions=result["Placemarks"], original=-1, selection=0, list_control=1, title=urllib.quote(__language__( 10005 ).encode('utf-8')) )
			self.ziel = ch.selection
			del ch
		self.proc_route_result(self.planRoute( self.start, self.ziel))
		self.xbmcearth_communication.get_Google_Analytics( referer_url,  urllib.quote("Search_Route".encode('utf-8')), urllib.quote(("/xbmc_earth/search_route.html?q="+str(self.start)+" - " + str(self.ziel) + "&cat=Route_Search").encode('utf-8')),self)
	...
	def proc_route_result(self, result):
		...
		list_item.setProperty('name', urllib.quote(result["name"].encode('utf-8')))
	...
	def checkGoogle_StreetView(self):
		Lon = self.lon
		Lat = self.lat
		Zoom = self.zoom
		googleearth_coordinates = Googleearth_Coordinates()
		...
		text = ''
		for node in nodelist:
			text = node.getAttribute("pano_id").encode('latin-1', 'ignore')
		if text !='':
			self.getControl(122).setEnabled(True)
		else:
			self.getControl(122).setEnabled(False)
	...
	def showPanoramio(self):
		...
		self.xbmcearth_communication.get_Google_Analytics( referer_url,  urllib.quote("Show Panoramio".encode('utf-8')), urllib.quote("/xbmc_earth/panoramio.html".encode('utf-8')),self)
	...
	def add_panoramio_hit(self, photo):
		...
		list_item.setProperty('Title', urllib.quote(photo["name"].encode('utf-8')))
		list_item.setProperty('Genre', urllib.quote(("Upload Date: " + photo["upload_date"] + " - Owner: " + photo["owner_name"]).encode('utf-8')))
	...
	def showFlickr(self):
		...
		self.xbmcearth_communication.get_Google_Analytics( referer_url,  urllib.quote("Show Flickr".encode('utf-8')), urllib.quote("/xbmc_earth/flickr_bbox.html".encode('utf-8')),self)
		self.piccontainer["enable"] = 1
	...
	def add_flickr_hit(self, photo):
		...
		list_item.setProperty('Piclink', TEMPFOLDER + photo["path"]+str(photo["photo_id"])+".jpg")
		list_item.setProperty('Title', urllib.quote(photo["name"].encode('utf-8')))
		list_item.setProperty('Genre', urllib.quote(("Date taken: " + photo["upload_date"] + " - Owner: " + photo["owner_name"]).encode('utf-8')))
		list_item.setProperty('lon',str(photo["lon"]))
		list_item.setProperty('lat',str(photo["lat"]))
	...
	def showLocr(self):
		...
		self.xbmcearth_communication.get_Google_Analytics( referer_url,  urllib.quote("Show Locr".encode('utf-8')), urllib.quote("/xbmc_earth/locr.html".encode('utf-8')),self)
		self.piccontainer["enable"] = 1
	...
	def add_locr_hit(self, photo):
		...
		list_item.setProperty('Piclink', TEMPFOLDER +"/"+ photo["path"]+str(photo["photo_id"])+".jpg")
		list_item.setProperty('Title', urllib.quote(photo["name"].encode('utf-8')))
		list_item.setProperty('Genre', urllib.quote(("Create Date: " + photo["created"] + " - Owner: " + photo["owner_name"]).encode('utf-8')))
		list_item.setProperty('lon',str(photo["lon"]))
		list_item.setProperty('lat',str(photo["lat"]))
		list_item.setProperty('photo_file_url',photo["photo_file_url"])
		list_item.setProperty('photo_id',str(photo["photo_id"]))
	...
	def showYoutube(self):
		...
		self.xbmcearth_communication.get_Google_Analytics( referer_url,  urllib.quote("Show Youtube".encode('utf-8')), urllib.quote("/xbmc_earth/youtube.html".encode('utf-8')),self)
		self.piccontainer["enable"] = 1
	...
	def add_youtube_hit(self, video):
		...
		list_item.setProperty('Title', urllib.quote(video["name"].encode('utf-8')))
		list_item.setProperty('Genre', urllib.quote(("Create Date: " + video["created"]).encode('utf-8')))#+ " - Owner: " + video["owner_name"]))
	...
	def showWebcams(self):
		...
		self.xbmcearth_communication.get_Google_Analytics( referer_url,  urllib.quote("Show Webcams".encode('utf-8')), urllib.quote("/xbmc_earth/webcams.html".encode('utf-8')),self)
		self.piccontainer["enable"] = 1
	...
	def add_webcams_hit(self, webcam):
		...
		list_item.setProperty('Title', urllib.quote(webcam["name"].encode('utf-8')))
		list_item.setProperty('Genre', urllib.quote(("Date taken: " + webcam["upload_date"] + " - Owner: " + webcam["owner_name"]).encode('utf-8')))
	...
	def search_map(self, key_txt = ''):
		...
		if keyboard.isConfirmed():
			...
			self.drawSAT()
			self.xbmcearth_communication.get_Google_Analytics( referer_url,  urllib.quote("Search_Map".encode('utf-8')), urllib.quote(("/xbmc_earth/search_map.html?q="+search_string+"&cat=Map_Search").encode('utf-8')),self)
	...
	def makeRoute(self, LineString ):
		...
		if self.map_move == 1:
			...
			d.path((-(self.map_pos_x-self.pic_size),-(self.map_pos_y-self.pic_size)),path,p)
		d.flush()
...
class marker:
	...
	def add_Marker(self):
		...
		self.window.markerindex = self.window.markerindex + 1
		if self.window.markerstartindex + self.window.markerindex <= self.window.markerstartindex + self.window.markerindexsize:
			self.marker_pic = self.window.getControl(self.window.markerstartindex + self.window.markerindex)
			self.marker_pic.setPosition(pos[0],pos[1])
			self.marker_pic.setWidth(self.x_size)
			self.marker_pic.setHeight(self.y_size)
			self.marker_pic.setImage(self.pic_path)
			self.marker_pic.setVisible(1)
		#xbmcgui.ControlImage(pos[0],pos[1],self.x_size,self.y_size, self.pic_path)
	...
	def __del__(self):
		self.marker_pic.setVisible(0)
		if  self.window.markerstartindex + self.window.markerindex > self.window.markerstartindex:
			self.window.markerindex = self.window.markerindex - 1
		pass
...
class draw_sat(Thread):
	...
	def run(self):
		if self.window.hybrid <= 1:
			self.window.getControl(510).setVisible(True)
			googleearth_coordinates = Googleearth_Coordinates()
			Lon = self.window.lon
			Lat = self.window.lat
				...
				for y in range(self.window.map_size_y):
					...
					while thread_starter<10:
						try:
							...
							thread_starter +=1
					#current.join(100)
		elif self.window.hybrid ==2 or self.window.hybrid ==3:
			try:
				self.window.getControl(510).setVisible(False)
				#for x in range(self.window.map_size_x):
				#	for y in range(self.window.map_size_y):
...
class draw_map(Thread):
	...
	def run(self):
		if self.window.hybrid == 1:
			self.window.getControl(520).setVisible(True)
			googleearth_coordinates = Googleearth_Coordinates()
			Lon = self.window.lon
			Lat = self.window.lat
			Zoom = self.window.zoom
			...
			try:
				self.window.getControl(520).setVisible(False)
